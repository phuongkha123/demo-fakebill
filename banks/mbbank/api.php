...
function stringWidth($inputString, $fontSize) {
    // Chiều rộng của mỗi ký tự tương đối với font size
    $charWidth = $fontSize * 0.6; // Giả sử mỗi ký tự chiếm 60% chiều rộng của font

    // Độ dài của chuỗi là tổng chiều rộng của các ký tự
    $stringLength = strlen($inputString);
    $width = $stringLength * $charWidth;
...
function formatBankName($inputString) {
    // Tách chuỗi thành tên ngân hàng và code ngân hàng
    $parts = explode(',', $inputString);

    // Lấy tên ngân hàng và code ngân hàng
    $bankName = ucwords(strtolower(trim($parts[0])));
    $bankCode = strtoupper(trim($parts[1]));

    // Format lại chuỗi và trả về
...
if (isset($_GET['type'])) {
    ...
    if ($_POST['code'] == 'MB') $_POST['code'] = 'MBBank, MB';
    ...
    if (!empty($username) || $_GET['type'] == 'demo') {
        ...
        if ($_GET['type'] == 'demo') {
            ...
            if ($sodu >= $tiengoc) {
                ...
                DB::insert('notifications', [
                    'notifications' => 'Đã tạo 1 bill MB, số tiền trừ ' . $tiengoc,
                    'username' => $username,
                    'amount' => $tiengoc
                ]);
            }
        ...
        if ($_POST['theme'] == 'ios4g') {
            if ($_POST['theme_bill'] == 'macdinh') {
                $theme = 'macdinh/'.$tmbStr.'mb_new_4g.png';
        ...
        if ($_POST['theme'] == 'ioswifi') {
            if ($_POST['theme_bill'] == 'macdinh') {
                $theme = 'macdinh/'.$tmbStr.'mb_new_wifi.png';
            } else if ($_POST['theme_bill'] == 'priority') {
...
//        }
        ...
        if ($_POST['bdsd'] == '1') {
            ...
            if ($_POST['theme'] == 'ios4g') {
                //$theme = 'mb789.png';
                if ($_POST['theme_bill'] == 'macdinh') {
                    $theme = 'macdinh/'.$tmbStr.'mb_new_bd_4g.png';
                } else if ($_POST['theme_bill'] == 'priority') {
            ...
            if ($_POST['theme'] == 'ioswifi') {
                if ($_POST['theme_bill'] == 'macdinh') {
                    $theme = 'macdinh/'.$tmbStr.'mb_new_bd_wifi.png';
                } else if ($_POST['theme_bill'] == 'priority') {
        ...
        if ($sodu >= $tiengoc) {
            {
                ...
                if ($_POST['bdsd'] == '1') {
                    // canletrai($sourceImage, 20, 155, imagecolorallocate($sourceImage, 0, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/fonts/Inter/Inter-Medium.ttf', 'Thông báo biến động số dư', 140);
                    $text = "12, 34";
                    $numbers = explode(', ', $_POST['time_bill']);
                    canletrai($sourceImage, 24, 230-25, imagecolorallocate($sourceImage, 0, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/fonts/Inter/Inter-Regular.ttf', 'TK ' . substr($_POST['stkgui'], 0, 2) . 'xxx' . substr($_POST['stkgui'], -3) . '|GD: -' . number_format($_POST['amount']) . 'VND', 150);
                    //canletrai($sourceImage, 24, 282-25, imagecolorallocate($sourceImage, 0, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/fonts/Inter/Inter-Regular.ttf',  $numbers[1] .' '. substr($numbers[0], 0, -3) . ' |SD:' . number_format($_POST['sdc']) . 'VND|ND: ' . substr($_POST['noidung'], 0, 2) . '...', 150);
                    
                    $dongduoi = $numbers[1] .' '. substr($numbers[0], 0, -3) . ' |SD: ' . number_format($_POST['sdc']) . 'VND|ND: ' . $_POST['noidung'];
                    canletrai($sourceImage, 24, 282-25, imagecolorallocate($sourceImage, 0, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/fonts/Inter/Inter-Regular.ttf',  substr($dongduoi, 0, 32) . '...', 150);
                }
                if (strpos($_POST['theme'], 'ios') != false or $_POST['theme'] == 'ios4g' or $_POST['theme'] == 'ioswifi') {
                    ...
                    imagettftext($sourceImage, 28, 0, 105, 68, imagecolorallocate($sourceImage, 0, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/fonts/San Francisco/SanFranciscoText-Semibold_Fix.otf', trim($_POST['time_dt']));
                }
                ...
                //TIỀN
                canchinhgiua($sourceImage, 44, 542, imagecolorallocate($sourceImage, 9, 93, 166), $_SERVER['DOCUMENT_ROOT'] . '/fonts/AvertaStd/AvertaStd-Semibold.otf', number_format(trim($_POST['amount']), 0, ',', ',') . ' VND');
                
                //NGÀY GIỜ 
                canchinhgiua($sourceImage, 21, 601, imagecolorallocate($sourceImage, 96, 101, 107), $_SERVER['DOCUMENT_ROOT'] . '/fonts/AvertaStd/AvertaStd-Regular.otf', formatDateTime($_POST['time_bill']));
                
                
                // TÊN NGƯỜI NHẬN
                canchinhgiua($sourceImage, 25, 778, imagecolorallocate($sourceImage, 44, 46, 54), $_SERVER['DOCUMENT_ROOT'] . '/fonts/AvertaStd/AvertaStd-Semibold.otf', removeAccentsAndToUpper($_POST['name_nhan']));
                
                // BANK NHẬN + STK NHẬN
                $thongtinnhan = $_POST['code'].' - '.$_POST['stk_nhan'];
                $chieurong = stringWidth($thongtinnhan, 21);
                canchinhgiuawithpaddingleft($sourceImage, 21, 843, imagecolorallocate($sourceImage, 27, 43, 55), $_SERVER['DOCUMENT_ROOT'] . '/fonts/AvertaStd/AvertaStd-Regular.otf', $thongtinnhan, 40);
                
                //NOIDUNG
                canchinhgiua($sourceImage, 22, 906, imagecolorallocate($sourceImage, 27, 43, 55), $_SERVER['DOCUMENT_ROOT'] . '/fonts/AvertaStd/AvertaStd-Regular.otf', $_POST['noidung']);
...
// Chèn ảnh con đã co dãn vào hình ảnh chính
                if ($trongMB) {
                    imagecopy($sourceImage, $resizedChildImage, 414-($chieurong + 25)/2 - 30, 805, 0, 0, $newChildWidth, $newChildHeight);
