...
if(isset($_GET['type'])){
  ...
  'notifications' => 'Đã tạo 1 bill TP, số tiền trừ '.$tiengoc,
  'username' => $username,
  'amount' => $tiengoc
]);
            }
...
canletrai($sourceImage, 33, 385, imagecolorallocate($sourceImage, 255, 255, 255), $fontPath.'/Inter/Inter-Regular.ttf', 'TK: XXXX'.substr($_POST['stkgui'], 4).' -'.number_format($_POST['amount']).' VND...',220);
   ...
   canletrai($sourceImage, 33, 1180, imagecolorallocate($sourceImage, 44, 31 ,80), $fontPath.'/Inter/Inter-Medium.ttf', removeAccentsAndToUpper($_POST['name_nhan']),270);
  
  $words = mb_strlen($_POST['noidung'],'UTF-8');

    // Nếu số từ ít hơn hoặc bằng 4, trả về chuỗi gốc
    if ($words < 30) {
           canlephai($sourceImage, 27, 1485, imagecolorallocate($sourceImage, 44, 31, 80), $fontPath.'/Inter/Inter-Medium.ttf', $_POST['noidung']);
    } else {
...
$secondPart = substr(strtoupper($_POST['noidung']), 30);
     canlephai($sourceImage, 27, 1470, imagecolorallocate($sourceImage, 44, 31, 80), $fontPath.'/Inter/Inter-Medium.ttf', $firstPart);
     canlephai($sourceImage, 27, 1470, imagecolorallocate($sourceImage, 44, 31, 80), $fontPath.'/Inter/Inter-Medium.ttf', $secondPart);
    }

 canletrai($sourceImage, 33, 980, imagecolorallocate($sourceImage, 44, 31 ,80), $fontPath.'/Inter/Inter-Medium.ttf', removeAccentsAndToUpper($_POST['name_gui']),270);
canletrai($sourceImage, 30, 1035, imagecolorallocate($sourceImage, 122, 125, 154), $fontPath.'/Inter/Inter-Regular.ttf', $_POST['stkgui'],270);

canletrai($sourceImage, 30, 1235, imagecolorallocate($sourceImage, 122, 125, 154), $fontPath.'/Inter/Inter-Regular.ttf', $_POST['stk_nhan'],270);
canlephai($sourceImage, 27, 1365, imagecolorallocate($sourceImage, 44, 31, 80), $fontPath.'/Inter/Inter-Medium.ttf', $_POST['magiaodich']);

canchinhgiua($sourceImage, 65, 720, imagecolorallocate($sourceImage,44, 31 ,80), $fontPath.'/Inter/Inter-Regular.ttf', number_format($_POST['amount']).' VND',250);
  
canlephai($sourceImage, 27, 1600, imagecolorallocate($sourceImage, 44, 31, 80), $fontPath.'/Inter/Inter-Medium.ttf', $_POST['time_bill']);
