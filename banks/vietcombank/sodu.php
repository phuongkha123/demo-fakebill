                ...
                if ($_POST['type'] == 'demo' || !empty($username)) {
                        ...
                        $sodu = DB::queryFirstField("SELECT sodu FROM `users` WHERE username = '$username'");
                         if ($sodu >= $tiengoc) {
                             ...
                             DB::insert('notifications', [
                                 'notifications' => 'Đã tạo 1 bill số dư VCB, số tiền trừ ' . $tiengoc,
                                 'username' => $username,
                                 'amount' => $tiengoc
                             ]);
                         }
                    ...
                    if ($sodu >= $tiengoc) {
                        ...
                        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
                            ...
                            imagedestroy($avatar);
                            $desiredWidth = 75;  // Set desired width
                            $desiredHeight = 75; // Set desired height
                            $circularAvatar = imagescale($circularAvatar, $desiredWidth, $desiredHeight);
                        }
                        // Load the base image
                        $image = imagecreatefrompng($_SERVER['DOCUMENT_ROOT'] . '/banks/vietcombank/sodu/vcb-sodu.png');

                        if ($watermark == 1) {
                            // canchinhgiua($sourceImage, 25, 100, imagecolorallocate($sourceImage, 255, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/bank/fonts/San Francisco/SanFranciscoText-Semibold.otf', 'Ảnh này chỉ để xem demo');
                            // canchinhgiua($sourceImage, 25, 300, imagecolorallocate($sourceImage, 255, 0, 0), $_SERVER['DOCUMENT_ROOT'] . '/bank/fonts/San Francisco/SanFranciscoText-Semibold.otf', 'Vui lòng ấn vào nút' . "\n" . '"Tạo số dư" để xóa dòng chữ này' . "\n" . 'Đây chỉ là demo để xem trước khi tải');
                        ...
                        if (isset($circularAvatar)) {
                            imagecopy($image, $circularAvatar, 55, 500, 0, 0, imagesx($circularAvatar), imagesy($circularAvatar));
                            imagedestroy($circularAvatar); // Free memory used by the avatar
                        }

                        // Tạo ảnh mới
                        // $image = imagecreatefrompng($_SERVER['DOCUMENT_ROOT'] . '/bank/banks/vietcombank/sodu/vcb-sodu.png');

                        // Viết văn bản lên ảnh
                        imagettftext($image, 22, 0, 150, 545, imagecolorallocate($image, 255, 255, 255), $_SERVER['DOCUMENT_ROOT'] . '/fonts/Inter/Inter-SemiBold.ttf', $_POST['name']);
                        ...
                        // imagettftext($image, 22, 0, 250, 618, imagecolorallocate($image, 255, 255, 255), $_SERVER['DOCUMENT_ROOT'] . '/bank/fonts/Inter/Inter-SemiBold.ttf', $_POST['stk']);


                        // Đoạn mã cho số dư
                        $balance = $_POST['amount'];
                        $vndFont = $_SERVER['DOCUMENT_ROOT'] . '/fonts/Snv Becker Regular/Snv Becker Regular.ttf';
                        $balanceSize = 30;
                        ...
                        imagettftext($image, $balanceSize, 0, $balanceX, $balanceY, imagecolorallocate($image, 255, 255, 255), $vndFont, number_format($balance));

                        $vndX = $balanceX + $balanceWidth - 5;

                        // Thêm hình ảnh nhỏ (ví dụ eye.png) sau số dư
                        $smallImagePath = $_SERVER['DOCUMENT_ROOT'] . '/banks/vietcombank/sodu/eye.png';
                        $smallImage = imagecreatefrompng($smallImagePath);

                        $desiredWidth = 138;  // Độ rộng mong muốn của hình ảnh nhỏ
                        ...
                        if ($resizedSmallImage !== false) {
                            ...
                            $smallImageX = $vndX + 10; // Thêm khoảng cách sau VND
                        ...
                        // Đoạn mã cho số tài khoản
                        $accountNumber = $_POST['stk'];
                        $accountFont = $_SERVER['DOCUMENT_ROOT'] . '/fonts/Snv Becker Regular/Snv Becker Regular.ttf';
                        $accountSize = 25;
                        $spacing = 5; // Khoảng cách giữa các ký tự
                        ...
                        // Tính toán vị trí của nút sao chép
                        $vndX = $accountX + $charWidth - 10;

                        $smallImagePath = $_SERVER['DOCUMENT_ROOT'] . '/banks/vietcombank/sodu/copy.png';
                        $smallImage = imagecreatefrompng($smallImagePath);

                        $smallImageWidth = imagesx($smallImage);
                        $smallImageHeight = imagesy($smallImage);

                        $smallImageX = $vndX + 5; // Thêm khoảng cách sau số tài khoản
                        $smallImageY = $accountY - $smallImageHeight + 10;
                        imagecopy($image, $smallImage, $smallImageX, $smallImageY, 0, 0, $smallImageWidth, $smallImageHeight);
                        ...
                        if ($watermark == 0) {
                            $name = rand() * 10000000000;
                            $tempFilePath = $_SERVER['DOCUMENT_ROOT'] . '/uploads/fake_sodu.png';
                            ...
                            $downloadUrl = "/uploads/fake_sodu.png";
